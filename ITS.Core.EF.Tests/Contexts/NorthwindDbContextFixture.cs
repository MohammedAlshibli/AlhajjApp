...
namespace ITS.Core.EF.Tests.Contexts
{
    public class NorthwindDbContextFixture : IDisposable
    {
        ...
        public void Initialize(bool useInMemory = false, Action seedData = null)
        {
            if (useInMemory)
            {
            ...
            {
                _options = new DbContextOptionsBuilder<NorthwindDbContext>()
                    .UseSqlServer(@"Data Source=(localdb)\MSSQLLocalDB; Initial Catalog=Urf.Core; Integrated Security=True; MultipleActiveResultSets=True")
                    .Options;
            }
